(() => {
  ...
  const formatNumber = (value, suffix = '') => {
    ...
    return Number.isFinite(number) ? `${numberFormat.format(Math.round(number))}${suffix}` : '—';
  };
  const formatDate = value => {
    const date = new Date(value || '');
    const bad = Number.isNaN(date.getTime());
    const stale = bad || Date.now() - date.getTime() > STALE_AFTER_DAYS * 86400000;
    return { label: bad ? 'Last updated time unavailable' : `Last updated ${date.toLocaleString()}`, stale };
  };
  const valueMarkup = value => Array.isArray(value)
    ? `<ul class="stat-row__list">${value.map(item => `<li>${escapeHtml(item)}</li>`).join('')}</ul>`
    : `<div class="stat-row__value">${escapeHtml(value ?? '—')}</div>`;
  const renderFallback = (container, playerName, message) => {
    container.innerHTML = `<div class="stats-fallback"><strong>Stats are warming up for ${escapeHtml(playerName)}.</strong><span>${escapeHtml(message)}</span></div>`;
  };
  const renderDashboard = (container, data) => {
    ...
    const summaryTiles = [
      ...
      ['Deaths', formatNumber(summary.deaths)],
      ['Mob kills', formatNumber(summary.mobKills)],
      ['Player kills', formatNumber(summary.playerKills)],
      ['Distance traveled', formatNumber(summary.distanceTraveledBlocks, ' blocks')],
      ['Jumps', formatNumber(summary.jumps)]
    ];
